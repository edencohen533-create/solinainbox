...
beforeAll(async () => { return withOrganization("legacy", async () => {
  ...
  await systemDatabase.$executeRawUnsafe('TRUNCATE TABLE "solina_qa_20260923"."Campaign", "solina_qa_20260923"."DistributionList", "solina_qa_20260923"."Conversation", "solina_qa_20260923"."Template", "solina_qa_20260923"."ProviderCredential" CASCADE');
  await prisma.contact.deleteMany({ where: { phone: "+972509990090" } });
  for (const [index, consentStatus] of (["OPTED_IN", "OPTED_OUT", "UNKNOWN", "OPTED_IN"] as const).entries()) {
    await prisma.contact.update({ where: { id: `qa-contact-${index}` }, data: { lastMarketingAt: null, isBlocked: false, consentStatus } });
  }
...
afterAll(async () => { return withOrganization("legacy", async () => {
  await prisma.providerCredential.updateMany({ where: { provider: "meta_whatsapp_cloud_api" }, data: { isActive: false } });
  vi.unstubAllGlobals(); await systemDatabase.$disconnect();
}); });
it("starts once under concurrent requests and enforces representative isolation on real PostgreSQL", async () => { return withOrganization("legacy", async () => {
...
it("submits for approval, blocks pending templates, syncs approval and sends a real-shaped Meta template request", async () => { return withOrganization("legacy", async () => {
...
it("deduplicates inbound webhooks, opens the reply window and advances delivery monotonically", async () => { return withOrganization("legacy", async () => {
...
it("snapshots a list, schedules/pauses/resumes, excludes opt-outs and unknown consent, and sends each opted-in recipient once", async () => { return withOrganization("legacy", async () => {
  const template = await prisma.template.findUniqueOrThrow({ where: { organizationId_name_language: { organizationId: "legacy", name: "qa_submitted", language: "he" } } });
...
it("keeps the assigned representative when a customer returns after closing a conversation, and honors STOP", async () => { return withOrganization("legacy", async () => {
...
it("serializes representatives, retains request IDs and records ambiguous provider acceptance without resend", async () => { return withOrganization("legacy", async () => {
...
it("blocks a revoked Meta token", async () => { return withOrganization("legacy", async () => {
  const conv = await startConversation(admin, "qa-contact-3", agentA.user.id);
  const template = await prisma.template.findUniqueOrThrow({ where: { organizationId_name_language: { organizationId: "legacy", name: "qa_submitted", language: "he" } } });
...
it("loads only the newest 100 messages with stable ordering from a 205-message history", async () => { return withOrganization("legacy", async () => {
