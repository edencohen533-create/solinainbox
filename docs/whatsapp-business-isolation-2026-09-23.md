...
3. Drain old requests/workers, then apply `20260923110621_whatsapp_tenant_natural_keys.sql`. Only this step removes global natural-key indexes; the replacement composite indexes already exist.
...
- Phase 1 is applied in production: 23/23 tables have RLS and business policies, zero table grants to public/anon/authenticated, 262 contacts and 459 messages retained. Application deployment and phase 2 verification are pending.
