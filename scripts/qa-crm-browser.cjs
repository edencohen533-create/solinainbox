...
(async () => {
  ...
  try {
    ...
    await page.locator('summary').filter({ hasText: 'עריכת פרטי לקוח' }).click();
    await page.getByLabel('אחראי CRM', { exact: true }).selectOption('qa-agent-a');
    await page.getByLabel('סטטוס ליד', { exact: true }).selectOption('QUALIFIED');
    const ownerSaved = page.waitForResponse((res) => res.url().endsWith(`/api/contacts/${contact.id}`) && res.request().method() === 'PATCH');
    await page.getByRole('button', { name: 'שמור פרטי לקוח', exact: true }).click();
    assert.equal((await ownerSaved).status(), 200);
    await page.reload();
    await page.locator('summary').filter({ hasText: 'עריכת פרטי לקוח' }).click();
    assert.equal(await page.getByLabel('אחראי CRM', { exact: true }).inputValue(), 'qa-agent-a');
    assert.equal(await page.getByLabel('סטטוס ליד', { exact: true }).inputValue(), 'QUALIFIED');
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
    fs.mkdirSync('/tmp/solina-qa-browser', { recursive: true });
    await page.screenshot({ path: '/tmp/solina-qa-browser/crm-ownership-mobile.png', fullPage: true });
    ...
    assert.deepEqual(errors, []); console.log('PASS CRM owner/stage persistence, opted-out service thread, task creation/completion from inbox, persistence in customer card, mobile layout, zero outbound messages');
