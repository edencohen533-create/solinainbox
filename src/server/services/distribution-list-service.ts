...
export async function saveDistributionList(input: z.infer<typeof distributionListSchema>, actorId: string, id?: string) {
  return prisma.$transaction(async (tx) => {
    ...
    const list = id
      ...
      : await tx.distributionList.create({ data: { ...data, ...(members.length ? { members: { createMany: { data: members } } } : {}) } });
    await tx.auditLog.create({ data: { actorUserId: actorId, action: id ? "audience.updated" : "audience.created", entityType: "DistributionList", entityId: list.id, metadata: { mode: input.segment ? "segment" : "static", members: members.length } } });
    return list;
  }, { timeout: 30000 });
}
