...
export async function submitMetaTemplate(input: unknown) {
  ...
  let local;
  try {
    local = await prisma.template.create({ data: { name: data.name, language: data.language, category: data.category, body: data.body, variables, status: "DRAFT", providerAccountId: config.businessAccountId, syncError: "ההגשה בתהליך; אם אינה מסתיימת יש לסנכרן מול Meta לפני ניסיון נוסף" } });
  } catch (error) {
