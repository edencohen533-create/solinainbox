...
function RuleEditor({ rule, onChange, options, path }: { rule: AudienceRule; onChange: (rule: AudienceRule) => void; options: AudienceOptions; path: string }) {
  ...
  return <div className="grid min-w-0 gap-2 sm:grid-cols-2">
    ...
    {rule.field === "custom" && <Input aria-label={`שם שדה ${path}`} placeholder="שם השדה כפי שנשמר בכרטיס הלקוח" value={rule.key} onChange={(event) => onChange({ ...rule, key: event.target.value })} />}
    {(rule.field === "custom" || rule.field === "source") && <Input aria-label={`ערך תנאי ${path}`} value={rule.value} onChange={(event) => onChange({ ...rule, value: event.target.value })} />}
