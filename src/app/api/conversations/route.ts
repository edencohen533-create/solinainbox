...
export async function POST(request: Request) {
  ...
  const parsed = z.object({ contactId: z.string().min(1), agentId: z.string().min(1).nullable().optional() }).safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "פרטי שיחה לא תקינים" }, { status: 400 });
  try { return NextResponse.json({ conversation: await startConversation(session, parsed.data.contactId, parsed.data.agentId) }, { status: 201 }); }
  catch (error) {
