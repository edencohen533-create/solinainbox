...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const note = await prisma.$transaction(async (tx) => {
      ...
      const note = await tx.note.create({ data: { body: parsed.data.body, conversationId: id, contactId: conversation.contactId, authorId: session.user.id }, include: { author: { select: { id: true, name: true } } } });
      await tx.auditLog.create({ data: { actorUserId: session.user.id, action: "note.created", entityType: "Note", entityId: note.id, conversationId: id, metadata: {} } });
      return note;
    });
