...
export const PATCH = organizationRequest(async function(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rule = await prisma.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({ data: { actorUserId: session!.user.id, action: parsed.data.isActive ? "automation.activated" : "automation.paused", entityType: "AutomationRule", entityId: id } });
    return updated;
  });
