...
export const GET = organizationRequest(async function(request: Request) {
  ...
  ] } : {}] }, orderBy: { id: "asc" }, take: 10001, select: { name: true, phone: true, email: true, source: true, consentStatus: true, isBlocked: true, consentAt: true, consentSource: true, consentScope: true, owner: { select: { name: true } }, leadStage: true } });
  ...
  return new Response(csvRows([
    ["name", "phone", "email", "source", "consentStatus", "isBlocked", "consentAt", "consentSource", "consentScope", "crmOwner", "leadStage"],
    ...contacts.map((c) => [c.name, c.phone, c.email, c.source, c.consentStatus, c.isBlocked, c.consentAt?.toISOString(), c.consentSource, c.consentScope, c.owner?.name, c.leadStage]),
